...
func getContextWithToken(ctx context.Context, r *http.Request) context.Context {
	...
	if authHeader != "" {
		if token, ok := parseAuthToken(authHeader); ok {
			ctx = context.WithValue(ctx, mcpContext.TokenContextKey, token)
		}
	}

	// Parse system token from X-User-Token header
	userTokenHeader := r.Header.Get("X-User-Token")
	if userTokenHeader != "" {
		ctx = context.WithValue(ctx, mcpContext.UserTokenContextKey, userTokenHeader)
	}
